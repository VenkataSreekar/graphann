#!/usr/bin/env bash
#
# Radial Layer Graph benchmark on SIFT1M.
# Usage: ./scripts/run_sift1m_rlg.sh | tee tmp/results_rlg.txt
#
# Prerequisites: run ./scripts/run_sift1m.sh first so that
#   tmp/sift_base.fbin, tmp/sift_query.fbin, tmp/sift_gt.ibin exist.
#
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
DATA_DIR="$ROOT/tmp"
BUILD_DIR="$ROOT/build"

BASE_FBIN="$DATA_DIR/sift_base.fbin"
QUERY_FBIN="$DATA_DIR/sift_query.fbin"
GT_IBIN="$DATA_DIR/sift_gt.ibin"
RLG_INDEX="$DATA_DIR/sift_rlg_index.bin"
VAMANA_INDEX="$DATA_DIR/sift_index.bin"

# ─── 0. Pre-flight ────────────────────────────────────────────────────────────
echo "=== Pre-flight checks ==="
for f in "$BASE_FBIN" "$QUERY_FBIN" "$GT_IBIN"; do
    [ -f "$f" ] || { echo "ERROR: $f not found. Run ./scripts/run_sift1m.sh first."; exit 1; }
done
echo "Data files OK."
echo ""

# ─── 1. Build binaries ────────────────────────────────────────────────────────
echo "=== Step 1: Building project ==="
mkdir -p "$BUILD_DIR"
pushd "$BUILD_DIR" > /dev/null
cmake .. -DCMAKE_BUILD_TYPE=Release
cmake --build . --parallel
popd > /dev/null
echo ""

# ─── 2. Build RLG index ───────────────────────────────────────────────────────
echo "=== Step 2: Building Radial Layer Graph index ==="
echo "  R=32  L=75  alpha=1.2  gamma=1.5  m=2.0"
echo "  (alpha=1.2 is the RNG alpha, same as Vamana; was 0.5 before — now fixed)"
"$BUILD_DIR/build_rlg" \
    --data   "$BASE_FBIN" \
    --output "$RLG_INDEX" \
    --R 32 --L 75 --alpha 1.2 --gamma 1.5 --m 2.0
echo ""

# ─── 3. Search with RLG  ──────────────────────────────────────────────────────
echo "=== Step 3: Searching with RLG index ==="
"$BUILD_DIR/search_rlg" \
    --index   "$RLG_INDEX"  \
    --data    "$BASE_FBIN"  \
    --queries "$QUERY_FBIN" \
    --gt      "$GT_IBIN"    \
    --K 10                  \
    --L 10,20,30,50,75,100,150,200
echo ""

# ─── 4. Baseline: Vamana ──────────────────────────────────────────────────────
echo "=== Step 4: Vamana baseline ==="
if [ ! -f "$VAMANA_INDEX" ]; then
    echo "  Vamana index not found — building..."
    "$BUILD_DIR/build_index" \
        --data "$BASE_FBIN" --output "$VAMANA_INDEX" \
        --R 32 --L 75 --alpha 1.2 --gamma 1.5
fi
"$BUILD_DIR/search_index" \
    --index   "$VAMANA_INDEX" \
    --data    "$BASE_FBIN"    \
    --queries "$QUERY_FBIN"   \
    --gt      "$GT_IBIN"      \
    --K 10                    \
    --L 10,20,30,50,75,100,150,200
echo ""

# ─── 5. Shell multiplier sweep  ───────────────────────────────────────────────
echo "=== Step 5: m-value sweep (m = 1.5, 2.0, 2.5, 3.0) ==="
for M in 1.5 2.0 2.5 3.0; do
    IDX="$DATA_DIR/sift_rlg_m${M}.bin"
    echo ""
    echo "--- m = ${M} ---"
    if [ ! -f "$IDX" ]; then
        "$BUILD_DIR/build_rlg" \
            --data "$BASE_FBIN" --output "$IDX" \
            --R 32 --L 75 --alpha 1.2 --gamma 1.5 --m "$M"
    fi
    "$BUILD_DIR/search_rlg" \
        --index "$IDX" --data "$BASE_FBIN" \
        --queries "$QUERY_FBIN" --gt "$GT_IBIN" \
        --K 10 --L 10,20,50,100,200
done
echo ""

echo "=== Done! ==="
echo ""
echo "To plot: python3 scripts/plot_pareto.py tmp/results_rlg.txt"