...
#ifdef _OPENMP
        {
            std::lock_guard<std::mutex> lk(mtx_[cur.id]);
            neighbors = adj_[cur.id];
        }
#else
        neighbors = adj_[cur.id];
#endif
    ...
    int p, std::vector<Candidate>& cands) const
{
    ...
    for (int k = 0; k < K && (int)selected.size() < R_; ++k) {
        ...
        // Use a flat contiguous buffer to avoid allocating a vector<float> per candidate.
        const int shell_size = (int)shells[k].size();
        std::vector<float> dirs((size_t)shell_size * dim_);
        for (int i = 0; i < shell_size; ++i) {
            const float* cd = node(shells[k][i].id);
            float* dir = dirs.data() + (size_t)i * dim_;
        ...
        for (int i = 0; i < shell_size && (int)picked_in_shell.size() < b; ++i) {
            bool diverse = true;
            const float* dir_i = dirs.data() + (size_t)i * dim_;
            for (int j : picked_in_shell) {
                const float* dir_j = dirs.data() + (size_t)j * dim_;
                if (cos_sim(dir_i, dir_j, dim_) > alpha_ang_) {
                    diverse = false;
                    break;
                }
            }
...
void RLGIndex::build_pass(int L_build) {
    ...
    std::cout << "[RLG] build_pass using " << num_threads << " thread(s)\n";
#else
    std::cout << "[RLG] build_pass OpenMP disabled; running single-threaded\n";
#endif
    ...
    for (int idx = 0; idx < n_; ++idx) {
        if ((idx + 1) > 0 && (idx + 1) % 10000 == 0) {
#ifdef _OPENMP
            #pragma omp critical
            {
                std::cout << "[RLG] Pass progress: " << (idx + 1) << " / " << n_ << "\r" << std::flush;
            }
#else
            std::cout << "[RLG] Pass progress: " << (idx + 1) << " / " << n_ << "\r" << std::flush;
#endif
