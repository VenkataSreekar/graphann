...
public:
    ...
    // For search-only mode: set data reference (does not take ownership)
    // Call this after load() if data_ wasn't loaded from the index file
    void set_data(const float* data) {
        data_.assign(data, data + (size_t)n_ * dim_);
    }
