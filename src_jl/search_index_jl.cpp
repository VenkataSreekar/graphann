...
int main(int argc, char** argv) {
    ...
    if (gt.npts != queries.npts) {
        std::cerr << "Error: ground truth rows != number of queries" << std::endl;
        return 1;
    }
    if (gt.dims < K) {
        std::cerr << "Warning: ground truth has fewer neighbors than K" << std::endl;
        K = gt.dims;
    }

    uint32_t nq = queries.npts;
    std::cout << "\n=== Vamana-JL Search Results (K=" << K
              << ", working_dim=" << index.get_proj_dim() << ") ===" << std::endl;
